...
public class Transform
{
    public JArray TransformData(JArray inputData)
    {
        ...
        foreach (JObject item in inputData)
        {
            ...
            var attributeLabelLookup = BuildAttributeLabelLookup(item, variantsExist);
            ...
            if (variantsExist)
            {
                ...
                foreach (JObject variant in item["variantsData"]!)
                    variantsAttributesArray.Add(PopulateVariantAttributes(variant, item["sku"]!.ToString(), attributeLabelLookup));
    ...
    private JObject TransformItem(JObject item)
    {
        ...
        TransformBreadcrumbs(itemCopy);

        CleanupURL(itemCopy);

        FlattenAttributes(itemCopy);
        ...
        TransformStarRatingDistribution(itemCopy);
    ...
    private void FlattenAttributes(JObject itemCopy)
    {
    ...
    private void TransformBreadcrumbs(JObject itemCopy)
    {
        var breadcrumbs = new List<string> { "Home" };
        if (itemCopy["breadcrumbs"] != null && itemCopy["breadcrumbs"]!.Type == JTokenType.Array)
        {
            var arr = itemCopy["breadcrumbs"]!.ToObject<string[]>();
            if (arr != null && arr.Length > 0)
                breadcrumbs.AddRange(arr);
        }
        itemCopy["category"] = string.Join(">", breadcrumbs);
        for (int i = 1; i <= breadcrumbs.Count; i++)
            itemCopy[$"categoryLvl{i}"] = breadcrumbs[i - 1];
        itemCopy.Remove("breadcrumbs");
    }

    private void CleanupURL(JObject itemCopy)
    {
    ...
    private void TransformStarRatingDistribution(JObject itemCopy)
    {
    ...
    private Dictionary<string, Dictionary<string, string>> BuildAttributeLabelLookup(JObject item, bool variantsExist)
    {
        ...
        return attributeLabelLookup;
    }

    private JObject PopulateVariantAttributes(JObject variant, string baseSku, Dictionary<string, Dictionary<string, string>> attributeLabelLookup)
    {
        var fullSku = baseSku + BuildSKU(variant);
        var attributesDict = new JObject();
        ...
        return new JObject { [fullSku] = attributesDict };
    }
}